...
def standard_scale(xTrain, xTest):
    ...
    xTrain : nd-array with shape n x d
        Training data 
    xTest : nd-array with shape m x d        Test data 
    ...
    """
    # TODO FILL IN
    # with help from https://scikit-learn.org/stable/modules/preprocessing.html
    # and https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.html

    # First scale both. Then convert back into dataframes to return
    xTrain_c = xTrain.copy()
    xTest_c = xTest.copy()
    scaler = preprocessing.StandardScaler().fit(xTrain_c)
    xTrain_s = scaler.transform(xTrain_c)
    xTest_s = scaler.transform(xTest_c)
    df_xTrain_s = pd.DataFrame(xTrain_s, columns=xTrain.columns)
    df_xTest_s = pd.DataFrame(xTest_s, columns=xTest.columns)
    return df_xTrain_s, df_xTest_s
...
def minmax_range(xTrain, xTest):
    ...
    xTrain_c = xTrain.copy()
    xTest_c = xTest.copy()
    scaler = preprocessing.MinMaxScaler().fit(xTrain_c)  # use the MinMax scaler instead of the standard scalar
    xTrain_s = scaler.transform(xTrain_c)
    xTest_s = scaler.transform(xTest_c)
    df_xTrain_s = pd.DataFrame(xTrain_s, columns=xTrain.columns)
    df_xTest_s = pd.DataFrame(xTest_s, columns=xTest.columns)
...
def add_irr_feature(xTrain, xTest):
    ...
    """
    # TODO FILL IN
    # Create irrelevant data with mean=0, std=1, and of length training data
    xTrain_c = xTrain.copy()
    xTest_c = xTest.copy()
    irrTrain = np.random.normal(0, 1, len(xTrain_c))
    irrTest = np.random.normal(0, 1, len(xTest_c))
    xTrain_c['irrelevant'] = irrTrain
    xTest_c['irrelevant'] = irrTest
    return xTrain_c, xTest_c
